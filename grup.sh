...
function alta_grupo {
		...
		cat /etc/group | grep "SIAD" | cut -f 1,3 -d : | egrep [0-9]{4} | cut -f1 -d :
		echo -e "\n\n"
		...
		if [ -z "SIAD$NGRU" ] ; then
			...
			if ( cat /etc/group | cut -f 1 -d : | egrep -wq "$NGRU" ) ; then
				...
				sudo groupadd "SIAD$NGRU" 2>/dev/null
					if [ "$?" = 0 ] ; then
						echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Grupo "$NGRU" creado exitosamente.">>log_siad.txt
...
function baja_grupo {
		...
		cat /etc/group | grep "SIAD" | cut -f1,3 -d : | egrep [0-9]{4} | cut -f1 -d :
		echo -e "\n"
		...
		if [ -z "$nmgroup" ] ; then
			...
			if ( cat /etc/group | cut -f1 -d : | egrep -wq "$nmgroup" ) ; then 
				sudo groupdel "$nmgroup" 2>/dev/null
			if [ "$?" = 0 ] ; then 
				echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-El grupo: "$nmgroup" ha sido borrado exitosamente." >>log_siad.txt
...
function Mod_nom { 
		while true ; do
		echo -e "Grupos: \n\n"
		cat /etc/group | grep "SIAD" | cut -f 1,3 -d : | egrep [0-9]{4} | cut -f 1 -d :
		echo -e "\n"
			...
			if [ -z "$NNOM" ] ; then
				...
				if ( cat /etc/group | egrep -wq "$NNOM" ) ; then
					...
					clear
	   				...
	   				sudo groupmod -n "$NNOM" "$NOMV" 2>/dev/null
					if [ "$?" = 0 ] ; then
						clear
						echo "Se modifico el nombre del grupo "$NOMV" a "$NNOM" exitosamente."
						echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Grupo: "$NOMV" modifico su nombre a "$NNOM"." >>log_siad.txt 	
						sleep 2
						break
						clear
					else
			...
			if ( cat /etc/group | egrep -wq "$NOMV" ) ; then 
				...
				if [ -z "$NID" ] ; then
					...
					if [ `echo "$NID" | egrep [1-9][0-9]{3}` ] ; then
						...
						sudo groupmod -g "$NID" "$NOMV" 2>/dev/null
					if [ "$?" = 0 ] ; then
						echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Se modifico el ID del grupo "$NOMV" a "$NID" exitosamente.">>log_siad.txt
...
function ver_grupos {
		...
		i=`cat /etc/group | grep "SIAD" | cut -f 1,3 -d : | egrep -c [1-9][0-9]{3}`
		i=$((i+1))		
		sleep 1
		while [ "$w" -lt "$i" ] ; do
		echo -e "Grupo: `cat /etc/group | grep "SIAD" | cut -f 1,3 -d : | egrep [0-9]{4} | cut -f 1 -d : | sed -n "$w"p`\t\tID: `cat /etc/group | cut -f 1,3 -d : | egrep [0-9]{4} | cut -f 2 -d : | sed -n "$w"p`"
		w=$((w+1))		
		done		
		echo -e "\n\n"
}
...
echo "	  ||	 	 / /  | | | |_| |  / /       | |               ||"
echo "	  ||    	/_/   |_| |_____/ /_/        |_|               ||"
echo "          ||       _____   _____    _   _   _____   _____   _____      ||"
echo "          ||      /  ___| |  _  \  | | | | |  _  \ /  _  \ /  ___/     ||"
echo "          ||      | |     | |_| |  | | | | | |_| | | | | | | |___      ||"
echo "          ||      | |  _  |  _  /  | | | | |  ___/ | | | | \___  \     ||"
echo "          ||      | |_| | | | \ \  | |_| | | |     | |_| |  ___| |     ||"
echo "          ||      |_____/ |_|  \_\ \_____/ |_|     \_____/ /_____/     ||"
echo "          ||                                                           ||"
echo "	  ||		                                               ||"
echo "	  ||			      Opciones:                        ||"
echo "	  || 			1) Alta de grupo                       ||"
echo "	  ||			2) Baja de grupo                       ||"
echo "	  ||			3) Modificacion de grupo               ||"
echo "	  ||			4) Ver grupos	                       ||"
echo "	  ||                                                           ||"
echo "	  ||			0) Salir		               ||"
echo "	   \___________________________________________________________/"
