...
function usu_existe {
		...
		cat /etc/passwd | grep "SIAD" | cut -d : -f1 | more
		echo -e "\t \t \n"
		while true ; do
...
function select_group {
		echo -e ""$1""
		cat /etc/group | grep "SIAD" | egrep [0-9]{3} | cut -f1 -d : | more
		echo -e "\nUsuario: "$usu""		
		read  -p "Grupo: " GROUP 
		if [ -z "$GROUP" ] ; then
...
function agregar_a_grupo { 
			...
			sudo gpasswd -a "$usu" "$GROUP" >> error.txt
			echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Se agrego al grupo: "$GROUP" el usuario: "$usu"" >>log_siad.txt
			elif [ "$1" = 2 ] ; then
			select_group "Seleccione el grupo en el que se quiere dar de baja:\n"
			sudo gpasswd -d "$usu" "$GROUP"
			echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Se borro del grupo: "$GROUP" el usuario: "$usu"" >>log_siad.txt
			else
			echo "Error"
			sleep 3		
			fi			
...
function alta_usuario {
				...
				case "$op" in
				1)clear
				sudo useradd -l "SIAD$usu" -d /home/"SIAD$usu" -m -c ""$NOM"-"$APE"-"$VCIV"" -s /bin/bash 2>/dev/null
sudo passwd SIAD$usu 
				if [ "$?" = 0 ] ; then 
					echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Usuario: "$usu" creado exitosamente." >>log_siad.txt
				...
				select_group 
				GROUPV=$GROUP
				sudo useradd -l "SIAD$usu" -d /home/"SIAD$usu" -m -c ""$NOM" "$APE" "$VCIV"" -p "$contra" -g "$GROUPV" -s /bin/bash 2>/dev/null 
passwd SIAD$usu
				if [ "$?" = 0 ] ; then 
					echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Usuario: "$usu" perteneciente al Grupo: "$GROUP" creado exitosamente" >>log_siad.txt
					echo "`date +%d/%m/%Y` - `date +%H:%M:%S` - Usuario "$usu" creado exitosamente."
					sleep 2
					clear
				else
				...
				*) clear
				...
				1) clear 
				agregar_a_grupo 1
				echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-El usuario: "$usu" se ha agregado correctamente al grupo: "$GROUP"" >>log_siad.txt
...
function baja_usuario {
			...
			cat /etc/passwd | grep "SIAD" | cut -f 1,3 -d : | egrep [0-9]{4}
			echo -e "\n\n"
			...
			if [ -z "$IDUSU" ] ; then
				...
				if ( cat /etc/passwd | cut -f3 | egrep -wq "$IDUSU" ) ; then
					...
					if [ $? -ne 0 ] ; then
						...
						echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Usuario "$usu" eliminado exitosamente. ">>log_siad.txt
						sleep 2
						clear
					fi		
				else
...
function modific_nom {
			...
			cat /etc/passwd | grep "SIAD" | cut -f1 -d : | more
			...
			if [ -z "$usu" ] ; then
				...
				if ( cat /etc/passwd | grep "SIAD" | egrep -wq "^"$usu"" ) ; then
				...
				if [ -z "$nnom" ] ; then
					...
					if ( cat /etc/passwd | grep "SIAD" | egrep -wq "$nnom" ) ; then
					echo "Este nombre ya esta siendo usado"
					sleep 2
					clear
					else
					sudo usermod -d /home/"$nnom" -m -l "SIAD$nnom" "$usu" 2>/dev/null
						if [ "$?" -ne 0 ] ; then
							echo "Ha ocurrido un error"
							sleep 2
							clear
						else
							echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Usuario "$usu" modifico su nombre a "$nnom".">>log_siad.txt
...
sudo passwd $usu
				echo "`date +%d/%m/%Y`-`date +%H:%M:%S`-Usuario "$usu" modifico su contraseña.">>log_siad.txt
}
...
function ver_usuarios {
		...
		i=`cat /etc/passwd | grep "SIAD" | cut -f 1,3 -d : | egrep -c [1-9][0-9]{3}`
		i=$((i+1))		
		sleep 1
		while [ "$w" -lt "$i" ] ; do
		echo -e "Usuario: `cat /etc/passwd | grep "SIAD" | cut -f 1,3 -d : | egrep [0-9]{4} | cut -f 1 -d : | sed -n "$w"p`\t\tID: `cat /etc/passwd | cut -f 1,3 -d : | egrep [0-9]{4} | cut -f 2 -d : | sed -n "$w"p`"
		w=$((w+1))		
		done		
		echo -e "\n\n"
}
